codeunit 50061 "XML DOM Mgt."
{
    ...
    procedure AddRootElement(var pXMLDocument: XmlDocument; pNodeName: Text; pNameSpace: text; var pCreatedNode: XmlNode): Boolean
    begin
        //pCreatedNode := XmlElement.Create(pNodeName).AsXmlNode(); 
        if pNameSpace > '' then
            pCreatedNode := XmlElement.Create(pNodeName, pNameSpace).AsXmlNode() // added pNameSpace
        else
            pCreatedNode := XmlElement.Create(pNodeName).AsXmlNode();
